...
const FeedsPage = () => {
    ...
    try {
      ...
      if (newPosts.length === 0) {
        ...
        setPosts((prevPosts) => {
          ...
          // Sort posts by date (newest first) and then randomize the order
          return allPosts
  ...
  const handleComment = async (postId) => {
    const comment = commentText[postId]; // Extract comment for specific post
    console.log("Comment:", comment);

    if (!comment || comment.trim() === "") return; // Prevent sending empty comments

    try {
      const response = await axios.post(
        `http://localhost:5000/api/posts/${postId}/comment`,
        { content: comment }, // Send comment text to the backend
        {
          headers: {
            "Content-Type": "application/json",
      ...
      const newComment = response.data; // Backend response with the new comment
      setPosts((prevPosts) =>
        prevPosts.map((post) =>
          post._id === postId
            ? {
                ...post, // Use immutability for the post update
                comments: [...post.comments, newComment], // Add the new comment
                totalComments: post.totalComments + 1, // Increment total comments
              }
            : post
        )
      );

      // Clear the comment input field after a successful post
